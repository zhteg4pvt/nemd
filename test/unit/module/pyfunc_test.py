...
class TestPress:
    ...
    @pytest.fixture
    def press(self):
        return lmpfunc.Press(os.path.join(BASE_DIR, 'press.data'))
...
class TestLength:
    ...
    @pytest.fixture
    def box_length(self):
        return lmpfunc.Length(os.path.join(BASE_DIR, 'xyzl.data'))
    ...
    def testPlot(self, box_length, tmp_dir):
        box_length.setData()
        box_length.setAve()
        box_length.plot()
...
class TestModulus:
    ...
    @pytest.fixture
    def modulus(self):
        return lmpfunc.Modulus(os.path.join(BASE_DIR, 'press.data'), 100)
    ...
    def testPlot(self, modulus, tmpdir):
        with lmpfunc.chdir(tmpdir, rmtree=True):
            modulus.setData()
            modulus.setAve()
            modulus.plot()
...
class TestScale:
    ...
    @pytest.fixture
    def scale(self):
        return lmpfunc.Scale(1, os.path.join(BASE_DIR, 'press.data'))
