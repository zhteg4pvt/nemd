...
class TestAmorphous:
    ...
    @pytest.fixture
    def cell(self, amorp, method, expected):
        amorp.options.method = method
        amorp.setMols()
        yield amorp
    ...
    @pytest.mark.parametrize(
        "argv,expected",
        [(['[Ar]'], [1, 4, 4, 4]),
         (['*C*', '-cru_num', '4', '-mol_num', '2'], [8, 11.73, 13.23, 8]),
         (['*C*', 'O', '-cru_num', '4', '1', '-mol_num', '2', '3'
           ], [17, 11.72, 13.23, 8])])
    def testSetGridded(self, cell, expected):
        cell.setGridded()

    @pytest.mark.parametrize('method', ['pack'])
    @pytest.mark.parametrize(
        "argv,expected",
        [(['[Ar]'], [1, 5.10, 5.10, 5.10]),
         (['*C*', '-cru_num', '4', '-mol_num', '2'], [8, 7.28, 7.28, 7.28]),
         (['*C*', 'O', '-cru_num', '4', '1', '-mol_num', '2', '3'
           ], [17, 8.27, 8.27, 8.27])])
    def testSetPacked(self, cell, expected):
        cell.setPacked()

    @pytest.mark.parametrize('method', ['grow'])
    @pytest.mark.parametrize(
        "argv,expected",
        [(['[Ar]'], [1, 5.10, 5.10, 5.10]),
         (['*C*', '-cru_num', '4', '-mol_num', '2'], [8, 7.28, 7.28, 7.28]),
    ...
    def testSetGrowed(self, cell, expected):
        cell.setGrowed()
    ...
    def testCreate(self, err_mock, run_mock, amorp, density, success,
                   expected):
